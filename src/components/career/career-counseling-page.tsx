  ...
  return blocks.map((b, bi) => {
    ...
    return (
      <ul key={bi} className="mt-4 space-y-4">
        {b.items.map((link) => (
          <li key={link.href} className="rounded-xl border border-[#d9c8b2] bg-white/70 p-4">
            <a
              href={link.href}
              target="_blank"
              rel="noopener noreferrer"
              className="font-semibold text-[#7b1e1e] underline-offset-4 hover:underline"
            >
              {link.label}
            </a>
            {link.description ? (
...
export function CareerCounselingPage({ page }: Props) {
  return (
    <div className="container-luxury space-y-10 py-10">
      <FadeIn className="card-luxury p-6 md:p-8">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#7b1e1e]">{page.eyebrow}</p>
            ...
            <p
              ...
              className={
                i === 0 && para === para.toUpperCase()
                  ? "text-sm font-semibold uppercase tracking-wide text-[#7b1e1e]"
        ...
        <div className="mt-6 flex flex-wrap gap-4 text-sm">
          <Link href="/tu-van-huong-nghiep" className="font-semibold text-[#7b1e1e] underline-offset-4 hover:underline">
            ← Tư vấn hướng nghiệp
          </Link>
        </div>
      </FadeIn>

      {page.youtubeEmbeds && page.youtubeEmbeds.length > 0 ? (
        <FadeIn className="card-luxury p-6 md:p-8" delay={0.03}>
          <h2 className="font-heading text-xl text-[#2f2018] md:text-2xl">Video</h2>
          <div
            className={`mt-4 grid gap-4 ${page.youtubeEmbeds.length === 1 ? "max-w-3xl" : "lg:grid-cols-2"}`}
          >
            {page.youtubeEmbeds.map((vid) => (
              <div
                key={vid.videoId}
                className={`overflow-hidden rounded-xl border border-[#d9c8b2]${page.youtubeEmbeds!.length === 1 ? " lg:mx-auto lg:w-full" : ""}`}
              >
            ...
            {page.externalLinks.map((link) => (
              <li key={link.href}>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center font-semibold text-[#7b1e1e] underline-offset-4 hover:underline"
                >
                  {link.label} ↗
                </a>
              </li>
      ...
      {page.cta ? (
        <FadeIn className="card-luxury border border-[#7b1e1e]/20 bg-[#faf7f2] p-6 md:p-8" delay={0.14}>
          ...
          {isExternalHref(page.cta.href) ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-4 inline-flex rounded-full bg-[#7b1e1e] px-6 py-3 text-sm font-medium text-[#f5efe6] transition hover:bg-[#5c1515]"
            >
              {page.cta.label} ↗
            </a>
          ) : (
          ...
          {careerCounselingNavItems
            ...
            .map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="block rounded-lg border border-[#d9c8b2] bg-white/60 px-4 py-3 text-sm text-[#5c4033]/90 transition hover:border-[#7b1e1e]/40 hover:bg-white"
                >
                  {item.label}
                </Link>
              </li>
